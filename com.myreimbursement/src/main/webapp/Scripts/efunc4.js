function getAwardedRequests() {
    let url = "http://18.216.101.201:8088/com.myreimbursement/Dispatcher/api/efunc4";
    let xhr = new XMLHttpRequest();
    let tbody = document.getElementById('infoHere');
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4 && xhr.status === 200){
            let awardedRequests = JSON.parse(xhr.responseText);
            for(let ar of awardedRequests["value"]){
...
let efunc4View = document.getElementById("efunc4View");
efunc4View.addEventListener("click", getAwardedRequests);