function getEmployeesPendingRequests() {
    let url = "http://18.216.101.201:8088/com.myreimbursement/Dispatcher/api/mfunc2";
    let xhr = new XMLHttpRequest();
    let tbody = document.getElementById('infoHere');
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4 && xhr.status === 200){
            let employeesPendingRequests = JSON.parse(xhr.responseText);
            for(let pr of employeesPendingRequests["value"]){
...
let mfunc2View = document.getElementById("mfunc2View");
mfunc2View.addEventListener("click", getEmployeesPendingRequests);