...
- **Oracle VirtualBox**: VirtualBox is a virtualization tool which allows for the creation of guest virtual machines on your host machine. We will use this tool to explore Docker as setting up Docker on Windows OS can be a headache.
- **Vagrant**: Vagrant is an open-source software which allows developers to build and maintain portable virtual software development environments. We will use Vagrant in conjunction with Oracle VirtualBox in order to easily provision and maintain virtual machines in our development environments.
